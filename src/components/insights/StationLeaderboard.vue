...
<template>
  <div class="rounded-xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
    ...
    <ul class="mt-2 space-y-1.5 text-sm text-slate-600">
      ...
      >
        <span class="truncate"
          >{{ pair.origin }} <span class="text-slate-400">→</span> {{ pair.destination }}</span
        >
    ...
    <div class="mt-5 rounded-lg bg-slate-50 p-3 text-sm text-slate-600">
      <span class="font-medium text-slate-800">{{ roundTrips.count }}</span> round trip{{
        roundTrips.count === 1 ? '' : 's'
      }}
