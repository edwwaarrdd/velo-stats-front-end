...
<template>
  <div class="rounded-xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
    ...
    <p class="mt-1 text-sm text-slate-500">
      Your dock-to-dock time against the ride time OSRM predicts (of {{ stats.ridesCompared }} with a cached
      route).
    </p>
    ...
    <div class="mt-4 grid grid-cols-2 gap-3 text-center sm:grid-cols-3">
      ...
      <div>
        ...
        <p class="text-xs text-slate-500">⏱ average {{ stats.averageDeltaSeconds < 0 ? 'gain' : 'loss' }}</p>
      </div>
    </div>

    <p class="mt-4 text-xs text-slate-400">
      The router assumes an uninterrupted ride, so traffic lights, junctions and docking the bike all count
      against you here.
    </p>
  </div>
</template>
