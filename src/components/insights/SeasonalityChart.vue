  ...
  <div class="rounded-xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
    ...
    <p class="mt-1 text-sm text-slate-500">
      <span v-if="hovered" class="font-medium text-slate-700">
        {{ hovered.label }}: {{ formatDistance(hovered.totalMeters) }} over {{ hovered.rideCount }} ride{{
          hovered.rideCount === 1 ? '' : 's'
        }}
      </span>
    ...
    <div class="flex gap-1 overflow-x-auto">
      <div v-for="month in months" :key="`label-${month.key}`" class="min-w-[10px] flex-1 text-center">
        ...
        <p class="text-[8px] leading-tight text-slate-300">
          {{ isJanuary(month.label) ? yearOf(month.label) : '' }}
        </p>
      </div>
    </div>
    ...
    <div class="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
      <div class="rounded-lg bg-emerald-50 p-3 text-sm text-emerald-800">
        ...
        <span v-if="best">
          {{ best.label }} — {{ formatDistance(best.totalMeters) }} over {{ best.rideCount }} rides.</span
        >
      </div>
      <div class="rounded-lg bg-slate-50 p-3 text-sm text-slate-600">
        ...
        {{ winter.rideCount }} rides, {{ formatDistance(winter.totalMeters) }},
        {{ formatDuration(winter.totalDuration) }} ({{ winter.percentageOfRides.toFixed(1) }}% of all rides).
      </div>
    </div>
  </div>
</template>
