  ...
  it('counts both origin and destination visits per station, sorted descending', () => {
    const rides = [
      makeRide({
        ...
        origin_station: 'Van Eyck',
        destination_station_code: '173',
        destination_station: 'Roosevelt',
      }),
      makeRide({
        origin_station_code: '173',
        origin_station: 'Roosevelt',
        destination_station_code: '041',
        destination_station: 'Van Eyck',
      }),
      makeRide({
        origin_station_code: '041',
        origin_station: 'Van Eyck',
        destination_station_code: '003',
...
describe('isRoundTrip / roundTripStats', () => {
  it('identifies a ride as a round trip only when origin and destination codes match and are non-null', () => {
    ...
    expect(isRoundTrip(makeRide({ origin_station_code: '041', destination_station_code: '173' }))).toBe(
      false,
    );
...
describe('hardcore ride detection', () => {
  ...
  it('combines rain, cold, and hot into a single isHardcore flag', () => {
    ...
    expect(
      rideHardcoreTags(makeRide({ weather: makeWeather({ temperature_c: 18, rain_mm: 0 }) })).isHardcore,
    ).toBe(false);
  });
